		...
		if(bCommandReady != 0)
		{
			...
			else if(*psz == 'D')
			{
				...
				else if(*psz == 'F')
				{
					...
					sscanf(psz,"%hu", &avar);
					pwm_div += ((uint32) avar);
					PWM_Clock_SetDividerValue(pwm_div);
				}
				else if(*psz == 'D')
				{
					// Set PWM Duty cycle
...
void Hardware_Config(void)
{
   ...
   // The extras
    VRef_Buf_Start(); // fixed 1.024 V reference output buffer
    AuxDAC_Buf_Start();
    AuxDAC_1_Start();
